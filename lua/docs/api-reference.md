...
`start` accepts only a `.ps1` file inside the active package directory and
returns immediately after creating the external process.

## Bundled assets

- `vesta.assets.weapon_font() -> string`

Возвращает байты того же TTF, который Vesta использует для штатных иконок
оружия и гранат. Скрипт может сохранить строку в бинарном режиме и использовать
font в локальном UI/Web UI без системных шрифтов и внешней загрузки.
